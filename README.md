...
- ✅ 代码完整：前端、后端、训练脚本全部写好
- ✅ 可直接运行：用现有数据微调后就能用
- ✅ **当前可用**：基于 Roboflow 龋齿数据集训练，可检测龋齿/蛀牙（4779+张训练图）。其他类别（牙菌斑、牙结石等）将在后续版本中逐步支持。
...
| 3 | Plaque | 牙菌斑 | 需补充数据 |
| 4 | Calculus | 牙结石 | 需补充数据 |
| 5 | Gingivitis | 牙龈炎 | 需补充数据 |
| 6 | Enamel_Wear | 釉质磨损 | 需补充数据 |

> 💡 当前使用 Roboflow 牙齿龋齿数据集（4779张训练 + 199张验证），后续将逐步采集并标注其他类别的数据，通过增量训练扩展检测能力。
