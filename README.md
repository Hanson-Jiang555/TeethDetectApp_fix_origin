...
3. **填写小程序 appid** — 在 rontend/project.config.json 中填写你的微信小程序 AppID
4. **配置后端地址** — 部署后修改 rontend/app.js 中的 aseUrl
...
1. rontend/app.js → 修改 aseUrl 为你的后端地址
   `js
   baseUrl: 'https://your-domain.com'  // 部署后的线上地址
   `

2. rontend/project.config.json → 填写你的 AppID
   `json
   "appid": "wx1234567890abcdef"  // 你的小程序AppID
   `

3. 用微信开发者工具打开 rontend/ 目录
4. 点击「预览」或「真机调试」
...
### Q: 小程序连不上后端？
A: 检查 pp.js 中的 aseUrl，本地调试用 http://localhost:8000，正式发布需要HTTPS域名。
